...
class MessageLabel: UILabel {
    ...
    internal lazy var rangesForDetectors: [Detector: [(NSRange, MessageTextCheckingType)]] = [:]
    ...
    public func setAttributes(_ attributes: [NSAttributedString.Key: Any], detector: Detector) {
        switch detector {
        case .phoneNumber:
    ...
    private func updateAttributes(for detectors: [Detector]) {
    ...
    private func detectorAttributes(for detectorType: Detector) -> [NSAttributedString.Key: Any] {
    ...
    private func parse(text: NSAttributedString) -> [NSTextCheckingResult] {
        ...
        // Get matches of all .custom Detector and add it to matches array
    ...
    private func parseForMatches(with detector: Detector, in text: NSAttributedString, for range: NSRange) -> [NSTextCheckingResult] {
        switch detector {
        case .custom(let regex):
            return regex.matches(in: text.string, options: [], range: range)
        default:
            fatalError("You must pass a .custom Detector")
        }
    }
    ...
    private func setRangesForDetectors(in checkingResults: [NSTextCheckingResult]) {
        ...
        for result in checkingResults {
            ...
            case .regularExpression:
                ...
                let detector = Detector.custom(regex)
  ...
  func handleGesture(_ touchLocation: CGPoint) -> Bool {
    ...
    private func handleGesture(for detectorType: Detector, value: MessageTextCheckingType) {
